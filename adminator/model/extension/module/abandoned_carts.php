...
class ModelExtensionModuleAbandonedCarts extends Model {
	...
	public function recoverEmail($order_id) {
		...
		$data['customer'] = ucfirst($order_info['firstname']);
		...
		foreach ($order_product_query->rows as $product) {
			$text .= $product['quantity'] . 'x ' . $product['name'] . "<br/>";
		...
		//$mail->setText($text); //plain text
		$mail->setHtml($this->load->view('mail/abandoned_cart', $data));
		$mail->send();
	...
	public function getOrders($data = array()) {
		...
		$sql .= " WHERE o.abandoned = '0' && o.date_added >= DATE_SUB(NOW(), INTERVAL ".$this->config->get('abandoned_carts_limit')." DAY) && NOW() >= DATE_ADD(o.date_added, INTERVAL 1 HOUR)  && o.firstname !='' && o.lastname !='' && o.order_status_id=0";
	...
	public function getTotalOrders($data = array()) {
		...
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` o WHERE o.date_added >= DATE_SUB(NOW(), INTERVAL ".$this->config->get('abandoned_carts_limit')." DAY) && NOW() >= DATE_ADD(o.date_added, INTERVAL 1 HOUR)  && o.firstname !='' && o.lastname !='' && o.order_status_id=0";
