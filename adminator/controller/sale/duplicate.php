<?php
class ControllerSaleDuplicate extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('sale/preorder');

		$this->document->setTitle("Duplicate");
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			'text' => "Duplicate",
			'href' => $this->url->link('sale/preorder', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);
		...
		$results = $this->model_sale_order->getDuplicate($filter_data);

		foreach ($results as $result) {
		    //var_dump($result); exit();
		    $idss = explode(",",$result['id']);
		    
		    $tempcon = '';
		    foreach ($idss as $ids) {
		        $tempcon = $tempcon . "<a href='/adminator/index.php?route=catalog/product/edit&product_id=". $ids ."&user_token=". $this->session->data['user_token'] . "'>". $ids ."</a> &nbsp;&nbsp;";
		    }
			...
			$data['orders'][] = array(
				...
				'sku'      => $result['sku'],
				'view'          => $this->url->link('sale/order/info', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['id'] . $url, true),
				'edit'          => $this->url->link('sale/order/edit', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['id'] . $url, true)
			);
		}
		...
		$data['sort_order'] = $this->url->link('sale/preorder', 'user_token=' . $this->session->data['user_token'] . '&sort=o.order_id' . $url, true);
		$data['sort_customer'] = $this->url->link('sale/preorder', 'user_token=' . $this->session->data['user_token'] . '&sort=customer' . $url, true);
		$data['sort_status'] = $this->url->link('sale/preorder', 'user_token=' . $this->session->data['user_token'] . '&sort=order_status' . $url, true);
		$data['sort_total'] = $this->url->link('sale/preorder', 'user_token=' . $this->session->data['user_token'] . '&sort=o.total' . $url, true);
		$data['sort_date_added'] = $this->url->link('sale/preorder', 'user_token=' . $this->session->data['user_token'] . '&sort=o.date_added' . $url, true);
		$data['sort_date_modified'] = $this->url->link('sale/preorder', 'user_token=' . $this->session->data['user_token'] . '&sort=o.date_modified' . $url, true);
		...
		$pagination->url = $this->url->link('sale/preorder', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('sale/duplicate_list', $data));
	}
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['emailinvoice'] = $this->url->link('sale/order/emailinvoice', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
    ...
    public function emailinvoice() {
		...
		$mail->setSubject('New Invoice TheLittleThings Order:'.$order_id );
		$mail->setHtml($this->load->view('sale/order_emailinvoice', $data));
		$mail->send();
		
		//remaining
		$this->response->setOutput($this->load->view('sale/order_emailinvoice', $data));
	}
