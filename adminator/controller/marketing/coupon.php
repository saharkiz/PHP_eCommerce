...
class ControllerMarketingCoupon extends Controller {
    ...
    {
        for ($i = 0; $i < 2; $i++) {
            $data['name'] = "Generated: " . $this->RandomString(5);
    		$data['code'] = 'COMIC-' . $this->RandomString(5);
    		$data['discount'] = rand(1,20);
    		$data['type'] = 'P'; 
    		$data['total'] = 1;
    		$data['logged'] = 1;
    		$data['shipping'] = 0;
    		...
    		$data['coupon_category'][0] = '1';
    		
    		$this->load->model('marketing/coupon');
            $this->model_marketing_coupon->addCoupon($data);
        }
		$this->session->data['success'] = "Coupons Generated Successful.";
	...
	protected function getList() {
        ...
        $data['generate'] = $this->url->link('marketing/coupon/generate', 'user_token=' . $this->session->data['user_token'] . $url, true);
