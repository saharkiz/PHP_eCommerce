...
class ControllerCatalogProduct extends Controller {
	...
	public function download() {
        ...
        header("Content-Disposition: attachment; filename=\"tlt_products_list.csv\"");
        $row = 'ProductID,Name,Model,Price,Qty,Status,SKU,Category,CategoryCode' . "\r\n";
		foreach ($results as $result) {
		    
		    $stat = $result['status'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled');
	...
	public function upload(){
	    //var_dump($this->request->files['import']);exit();
	     $filename = html_entity_decode($this->request->files['import']['name'], ENT_QUOTES, 'UTF-8');
	    if (strtolower(substr(strrchr($filename, '.'), 1)) != 'csv')
	    {
	        $json['error'] = 'NOT CSV. Please select a CSV File.';
		    echo $json['error'];
	        exit();
	    }
	    $content = file_get_contents($this->request->files['import']['tmp_name']);
	    $file = $filename . '.' . token(32);
	    move_uploaded_file($this->request->files['import']['tmp_name'], DIR_UPLOAD . $file);
        $this->load->model('catalog/product');
        $row = 1;
        if (($handle = fopen(DIR_UPLOAD .$file, "r")) !== FALSE) {
          while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
            $num = count($data);
            //echo "<p> $num fields in line $row: <br /></p>\n";
            $row++;
            $this->model_catalog_product->uploadProduct($data[0],$data[1],$data[2],$data[3],$data[4]);
          }
          fclose($handle);
        }
        else
        {
            $json['error'] = 'Cannot Read CSV File';
		    echo $json['error'];
	        exit();
        }
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'view'       => '/index.php?route=product/product&product_id=' . $result['product_id']
			);
		}
	...
	protected function getForm() {
		...
		if (isset($this->request->post['creditcard'])) {
			$data['creditcard'] = $this->request->post['creditcard'];
		} elseif (!empty($product_info)) {
			$data['creditcard'] = $product_info['creditcard'];
		} else {
			$data['creditcard'] = 1;
		}
        ...
        if (isset($this->request->post['deposit'])) {
			...
			$data['deposit'] = $product_info['deposit'];
		} else {
			$data['deposit'] = '';
		}
    ...
    {
        $this->load->model('catalog/product');
        $results = $this->model_catalog_product->isSkuAvailable($this->request->get['sku']);
        if ($results > 0)
        {
            echo '1';
        }
